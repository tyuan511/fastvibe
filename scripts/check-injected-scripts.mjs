...
const TARGET = join(here, "..", "src", "renderer", "src", "components", "layout", "side-pane-browser.tsx");

/** Script fragments held in constants and handed to `inject` by name. */
const FRAGMENT_CONSTANTS = ["PAGE_HELPERS", "SNAPSHOT_BODY"];
...
const scripts = collect(source);
const problems = [];

if (scripts.length < 4) {
  problems.push(
    `only ${scripts.length} injected script(s) found in ${TARGET} — the extractor no longer matches the code, so this check is not covering anything`,
  );
}
