...
function enqueueSettingsMutation<T>(operation: () => Promise<T>): Promise<T> {
  const next = settingsMutationQueue.then(operation, operation);
  settingsMutationQueue = next.then(() => undefined, () => undefined);
  return next;
}
 ...
 */
function writeArchived(change: (previous: string[]) => string[]): Promise<void> {
...
export function rememberPermission(prompt: PermissionPrompt): Promise<void> {
  return enqueueSettingsMutation(async () => {
    const remote = getClient();
    if (!remote) return;
