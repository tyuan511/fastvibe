...
3. **附带离视口最近的 60 个屏幕外控件**，标记 `offscreen`，并补一条规则说明它们可以直接操作，执行前自动滚进视口。原版只观察屏幕内，目标在首屏以下时 Jev 会直接判 BLOCKED（试点中 8 次失败里有 6 次）。翻页类控件（`rel=next`、“More”“下一页”“下一章”等）不论距离远近都会提供；规则里也写明，需要的控件不在列表中且页面还能滚动时，应当滚动而不是判 BLOCKED。HN 翻两页的任务由此从“第一步 BLOCKED”变为 2 步完成，循环 2.9 s，全程 6.9 s（主模型直接操作为 16.3 s）。
...
把决策层作为一个抽象工具交给主 agent：主 agent 写一道**封闭**问题（choice 选标签 / score 打分 / yes_no 是非），附上一组条目，决策模型逐条作答。适合的任务是大量同类条目的分诊与归类——issue、邮件、文件、日志行、测试失败、表格行列映射、搜索结果相关性筛选。开关：设置 → 决策引擎 › 应用场景 › 批量决策（`batchDecide`，缺省关闭）。

- **形状即约束**（`src/main/engine/decision/batch.ts`，纯模块）：不足 5 条直接拒绝，让主 agent 自己判断；最多 500 条、8 个问题；choice 至少两个选项且每个选项必须带一句含义；没有自由文本答案。条目 id 不可重复，单条超过 16k 字符的条目不发送、单独记为 failed。
...
**复杂任务（本地订酒店站点，一次完整预订）**：先搜索（自动补全里有干扰项，另有日期和人数），再到首屏以下的结果中筛选“免费取消 + 4 星”并按价格排序，避开两个干扰酒店，进入详情选房型，填 7 项表单（含日期、时间字段），取消默认勾选的促销订阅，同意条款并提交。由脚本逐项核对页面记录的预订数据。

| | Jev 循环 | 主模型直接操作 `browser_*` |
| --- | --- | --- |
| 结果 | 18 项全对，回答正确 | 全对（25 轮上限内未完成，放宽到 60 轮后用 44 轮完成） |
| 全程 | 23.4 s（循环 19.4 s） | 61.9 s |
| 大模型调用 / 输入 tokens | 9 次 / 约 4k | 45 次 / 约 320k |
| 决策 | Jev 24 次，约 0.3 s/次 | 主模型 44 轮 |

每种方式各跑 1 次。它说明的是：在多页面、多表单、多筛选的交互型任务上差距很大；真实网站的登录、弹窗、iframe 等情况还未覆盖。Jev 在确认页判了 BLOCKED 而不是 DONE（已无事可做），不影响结果。
