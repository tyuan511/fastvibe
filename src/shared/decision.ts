 ...
 * model drives; `jev` adds the `browser_task` loop. There is no secret in this
 ...
 */
export type DecisionModelConfig = { kind: "off" } | { kind: "jev" };
...
export function decisionModelConfigOf(value: unknown): DecisionModelConfig {
  ...
  return (value as { kind?: unknown }).kind === "jev" ? { kind: "jev" } : DEFAULT_DECISION_MODEL;
}
