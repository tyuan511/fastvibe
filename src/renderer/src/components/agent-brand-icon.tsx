...
/**
 * Brand marks for the agents FastVibe can import sessions from, so the 导入
 * pane reads as four products rather than four rows of text.
 *
 * Vendored from LobeHub's icon set (https://lobehub.com/icons, MIT) as `currentColor`
 * paths rather than <img src>: the marks are monochrome, so inlining them lets each
 * one follow the active theme's foreground instead of pinning a palette colour the
 * way a raster asset would (see the theming rules in AGENTS.md).
 *
 * Claude Code uses Claude's mark and Codex uses Codex's; both agents keep their own
 * brand because that is what the user recognises in the source tool.
 */
const PATHS: Record<ImportSourceId, JSX.Element> = {
  "claude-code": (
    <>
